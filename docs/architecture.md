...
A captain call deferred with `bin/fm-captain-hold.sh hold --until <date>` is the exception: the watcher asks `open --deferred` only when that recheck, or a held call's stale alarm, is about to ring, and the away-mode daemon asks it before its own captain-held recheck; each spends the due window silently while tasks-axi still reports the hold active, and resumes the ordinary cadence on the first window on or after the date.
