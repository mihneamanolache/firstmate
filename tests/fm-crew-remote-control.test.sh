...
# The brief file the launch line reads is the spawn's, not this feature's: a
# no-mistakes ship is delivered the rendered launch-brief.md while a scout is
# delivered its brief.md, so a scout case names its own.
claude_launch_line() {  # <home> <task-id> <flag-including-trailing-space-or-empty> [brief-basename]
  local brief=${4:-launch-brief.md}
  printf '%s' "env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\"}' $3\"\$('$ROOT/bin/fm-operational-input.sh' encode launch-brief < '$1/data/$2/$brief')\""
}
...
test_default_launches_a_scout_with_remote_control() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  expected=$(claude_launch_with_flag "$HOME_DIR" "$id" brief.md)
  [ "$launch" = "$expected" ] \
...
test_off_leaves_a_scout_launch_unchanged() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  [ "$launch" = "$(claude_launch_without_flag "$HOME_DIR" "$id" brief.md)" ] \
...
test_a_default_on_worker_is_still_steerable_through_the_durable_inbox() {
  local rec id out status launch record body doorbell typed
  id=rc-steer-g2
  ...
  out=$(FM_ROOT_OVERRIDE='' FM_HOME="$HOME_DIR" \
    ...
    FM_SEND_SETTLE=0 FM_FAKE_LAUNCH_LOG="$LAUNCH_LOG" PATH="$FAKEBIN_DIR:$PATH" \
  ...
  doorbell=$(printf 'Firstmate instruction waiting: list %s' "$(cd "$HOME_DIR/state/$id.inbox" && pwd)")
