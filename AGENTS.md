...
config/crew-remote-control  opt-OUT for the default-on Claude Code Remote Control that makes every CLAUDE crewmate and scout drivable from claude.ai/code; LOCAL, gitignored; absent, empty, or "on" = on, "off" = off, anything else warns and stays on; no other harness and no secondmate launch is ever affected; inherited by secondmate homes; see docs/configuration.md "Crewmate Remote Control"
