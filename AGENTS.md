...
config/crew-remote-control  opt-OUT for the default-on Remote Control on CLAUDE crewmate and scout launches, which makes each worker drivable from claude.ai/code under a per-launch "<task-id>.<home-basename>.<launch-token>" session name; LOCAL, gitignored; absent, empty, or "on" = on, "off" = off, anything else warns and stays on; a claude whose own --help does not advertise the option launches without the flag instead of failing; no other harness and no secondmate launch is ever affected; inherited by secondmate homes (docs/configuration.md)
